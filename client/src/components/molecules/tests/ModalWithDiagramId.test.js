...
const renderModalWithDiagramId = () => {
  ...
  const utils = render(<ModalWithDiagramId id={476} closeCallback={() => (state.modalIsOpen = false)} />);
  return { ...utils, state };
};
...
describe('ModalWithDiagramId', () => {
  it('renders with correct text', () => {
    ...
    expect(getByText('Twoje zadanie jest dostępne pod adresem localhost:3000/zadania/476')).toBeInTheDocument();
  });
